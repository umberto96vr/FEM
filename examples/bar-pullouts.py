...
for (k, NodesElement) in enumerate([2,3]):
    for (i, alfa) in enumerate(alfa_range):
        ...
        for (j,n) in enumerate(n_cases):
            ...
            mesh.elements     = element_table(1, 5, mesh)
            mesh.coordinates  = np.linspace(0, L, mesh.Nodes)
            ...
            if elem_plot == True:
                ...
                plt.plot(mesh.coordinates, U, '-xb')
                true_sol = -P/(E*A*alfa) * np.exp(-alfa*mesh.coordinates)
                plt.plot(mesh.coordinates, true_sol, '-+r')
