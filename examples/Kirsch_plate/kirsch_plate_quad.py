...
H = [1.1]#1.1, 0.49, 0.24, 0.12, 0.1, 0.09]#, 0.085, 0.078, 0.07]
ERRORE  = []
ELEMEN  = []
DOFS    = []
REL_ERR = []
...
for h in H:
    ...
    if mesh_file != 'kirsch_plate_full':
    ...
    for e in range(len(mesh.elements)):
        ...
        if key['stiffness matrix']["evaluation"] == 'numerical integration':
            ...
            gauss_points_number = 1#key['stiffness matrix']["points"]
        ...
        for i in range(len(int_p)):
            for j in range(len(int_p)):
                ...
                (detJ, dN, N) = FEM_utilities.shape_functions(elementPoints, int_p_vect, 
                                                              nodesInElement, mesh.d)
                x_c = np.dot(elementPoints[:,0],N[0,:])
                y_c = np.dot(elementPoints[:,1],N[0,:])
